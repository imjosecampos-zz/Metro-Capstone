...
function topshop_theme_update_filter( $checked_data ) {
	...
	$raw_response = wp_remote_post( TOPSHOP_UPDATE_URL . '/premium/' . $theme . '/' , $send_for_check );
...
add_filter( 'pre_set_site_transient_update_themes', 'topshop_theme_update_filter' );


// Take over the Theme info screen on WP multisite
add_filter( 'themes_api', 'topshop_api_call', 10, 3 );

function topshop_api_call( $def, $action, $args ) {
	...
	$request = wp_remote_post( TOPSHOP_UPDATE_URL . '/premium/' . $theme . '/', $request_string );

	if ( is_wp_error( $request ) ) {
		$res = new WP_Error( 'themes_api_failed', __( 'An Unexpected HTTP Error occurred during the API request.</p> <p><a href="?" onclick="document.location.reload(); return false;">Try again</a>', 'topshop' ), $request->get_error_message() );
	} else {
		$res = unserialize( $request['body'] );

		if ( $res === false )
			$res = new WP_Error( 'themes_api_failed', __( 'An unknown error occurred', 'topshop' ), $request['body'] );
	}
